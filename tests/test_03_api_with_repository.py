...
def test_create_valid_book_successfully(app, client, a_book) -> None:
    ...
    response_new_book = client.post(url_create_book, json={"book": a_book.model_dump()})
...
def test_get_book_by_id_successfully(app, client, a_book, another_book) -> None:
    url_create_book = app.url_path_for("books:create-book")
    response_new_book_1 = client.post(url_create_book, json={"book": a_book.model_dump()})
    print(response_new_book_1.json())
    assert response_new_book_1.status_code == 201
    book_1 = response_new_book_1.json()
    book1_id = book_1['id']

    response_new_book_2 = client.post(url_create_book, json={"book": another_book.model_dump()})
